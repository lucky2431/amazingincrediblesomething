(function() {
  ...
  if (Defined.rchtype === undefined) {
    ...
    if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
      ...
      net.silent('http://durex.monster'.indexOf(location.host) >= 0 ? 'https://github.com/' : 'http://durex.monster/cors/check', function() {
        check(true);
      }, function() {
        check(false);
      }, false, {
  ...
  function component(object) {
    ...
    this.rch = function(json, noreset) {
      ...
      if (typeof signalR == 'undefined') {
        Lampa.Utils.putScript(["http://durex.monster/signalr-6.0.25_es5.js"], function() {}, false, function() {
          load();
        }, true);
      } else load();
    };
  ...
  function startPlugin() {
    ...
    var manifst = {
      ...
      onContextLauch: function onContextLauch(object) {
        resetTemplates();
        Lampa.Component.add('durex', component);
    ...
    Lampa.Component.add('durex', component); //то же самое
    resetTemplates();
  ...
  if (!window.durex_plugin) startPlugin();
